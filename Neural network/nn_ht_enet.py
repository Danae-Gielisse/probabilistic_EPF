...
Run this file to perform the hyperparameter tuning for the DDNN elastic net methods.
"""
...
def objective(trial):
    try:
        ...
        for train_no in range(val_multi):
            ...
            h1_activation_rate_l1 = (0.0 if not regularize_h1_activation
                                     else trial.suggest_float('h1_activation_rate_l1', 1e-5, 1e1, log=True))
            h1_activation_rate_l2 = (0.0 if not regularize_h1_activation
                                     else trial.suggest_float('h1_activation_rate_l2', 1e-5, 1e1, log=True))
            h1_kernel_rate_l1 = (0.0 if not regularize_h1_kernel
                                 else trial.suggest_float('h1_kernel_rate_l1', 1e-5, 1e1, log=True))
            h1_kernel_rate_l2 = (0.0 if not regularize_h1_kernel
                                 else trial.suggest_float('h1_kernel_rate_l2', 1e-5, 1e1, log=True))
            ...
            hidden = tfk.layers.Dense(trial.suggest_int('neurons_1', 16, 1024, log=False),
                                      ...
                                      kernel_regularizer=tfk.regularizers.L1L2(l1=h1_kernel_rate_l1,
                                                                               l2=h1_kernel_rate_l2),
                                      activity_regularizer=tfk.regularizers.L1L2(l1=h1_activation_rate_l1,
                                                                                 l2=h1_activation_rate_l2),
                                      dtype=tf.float32)(last_layer)
            ...
            h2_activation_rate_l1 = (0.0 if not regularize_h2_activation
                                     else trial.suggest_float('h2_activation_rate_l1', 1e-5, 1e1, log=True))
            h2_activation_rate_l2 = (0.0 if not regularize_h2_activation
                                     else trial.suggest_float('h2_activation_rate_l2', 1e-5, 1e1, log=True))
            h2_kernel_rate_l1 = (0.0 if not regularize_h2_kernel
                                 else trial.suggest_float('h2_kernel_rate_l1', 1e-5, 1e1, log=True))
            h2_kernel_rate_l2 = (0.0 if not regularize_h2_kernel
                                 else trial.suggest_float('h2_kernel_rate_l2', 1e-5, 1e1, log=True))
            ...
            hidden = tfk.layers.Dense(trial.suggest_int('neurons_2', 16, 1024, log=False),
                                        ...
                                        kernel_regularizer=tfk.regularizers.L1L2(l1=h2_kernel_rate_l1,
                                                                                 l2=h2_kernel_rate_l2),
                                        activity_regularizer=tfk.regularizers.L1L2(l1=h2_activation_rate_l1,
                                                                                   l2=h2_activation_rate_l2),
            ...
            if paramcount[distribution] is None:
                ...
                for p in range(paramcount[distribution]):
                    ...
                    param_kernel_rate_l1 = (0.0 if not regularize_param_kernel
                                            else trial.suggest_float(param_names[p] + '_rate_l1', 1e-5, 1e1, log=True))
                    param_kernel_rate_l2 = (0.0 if not regularize_param_kernel
                                            else trial.suggest_float(param_names[p] + '_rate_l2', 1e-5, 1e1, log=True))
                    param_layers.append(tfk.layers.Dense(
                        24, activation='linear', # kernel_initializer='ones',
                        kernel_regularizer=tfk.regularizers.L1L2(l1=param_kernel_rate_l1, l2=param_kernel_rate_l2))(hidden))
...
study_name = f'FINAL_NL_selection_prob_{distribution.lower()}{run}_enet_ts{time_span}'
